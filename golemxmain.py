...
bot = telebot.TeleBot('7703277028:AAG75vutgCokqH7a1Ah20sx6UxozC47lB0w')
...
def generate_key(duration):
    ...
    return f"GOLEM-{duration.upper()}-{random_part}"
...
def start_command(message):
    ...
    bot.reply_to(message, "𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 𝙂𝙊𝙇𝙀𝙈 𝘿𝘿𝙊𝙎 𝙋𝘼𝙄𝘿 𝘽𝙊𝙏!", reply_markup=markup)
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "⛔️ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM @GOLEM_OWNER"
        bot.reply_to(message, response)
...
def status_command(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "⛔️ 𝗬𝗼𝘂 𝗮𝗿𝗲 𝗻𝗼𝘁 𝗮𝗻 𝗮𝗱𝗺𝗶𝗻 @GOLEM_OWNER.")
...
def upload_to_terminal(message):
    """Handle file upload to terminal for admins @GOLEM_OWNER."""
    user_id = str(message.chat.id)
...
def add_user_command(message):
    if str(message.chat.id) not in admin_id:
        bot.reply_to(message, "⛔ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM @GOLEM_OWNER")
        return
...
if __name__ == "__main__":
    print("✅ BOT IS STARTING FILE DEVELOPER @GOLEM_OWNER!... ")
    while True:
        load_data()
        try:
